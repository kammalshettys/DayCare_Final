...
//        this.createGroups(schoolInstance);
        ...
        for(ClassRoom classRoomObj : classRoomList){
            for ( Teacher teacherObj : teacherList){
                ...
                classRoomObj.getTeacherStudentGroup().put(teacherObj, new ArrayList<Student>());
                
                if (classRoomObj.getTeacherStudentGroup().size() == 3){
                    break;
                }
        ...
        for(ClassRoom classRoomObj : classRoomList){
            for (Map.Entry entry : classRoomObj.getTeacherStudentGroup().entrySet()){
                ...
                classRoomObj.getTeacherStudentGroup().put((Teacher) entry.getKey(), listToAddData);
            } 
        }
